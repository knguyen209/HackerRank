...
function cookies(k: number, A: number[]): number {
    let minHeap = new MinBinaryHeap(A)
    minHeap.buildMinHeap()
    let count = 0
    while (minHeap.getMinVal() < k) {
        if (minHeap.getSize() == 1) {
            return -1
        }
        let firstLeast = minHeap.extractMin() || 0
        let secondLeast = minHeap.extractMin() || 0
        let val = firstLeast + secondLeast * 2
        minHeap.insert(val)
        count++
    }
...
// let count = cookies(7, [1, 2, 3, 9, 10, 12])
// console.log(count)
let count = cookies(10, [1, 1, 1])
console.log(count)
